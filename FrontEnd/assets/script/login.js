...
function login(id) {
    console.log(id);
    loginEmailError.insertAdjacentHTML("afterbegin", "");
    loginMdpError.insertAdjacentHTML("afterbegin", "");
    // véeification de l'email
    if (!id.email.match(/^[a-zA-Z0-9._-]+@[a-zA-Z0-9._-]{2,}\.[a-z]{2,4}$/g)) {
        const p = document.createElement("p");
        p.insertAdjacentHTML("afterbegin", "Veuillez entrer une addresse mail valide");
    ...
    if (id.password.length < 5 && !id.password.match(/^[a-zA-Z0-9]+$/g)) {
        const p = document.createElement("p");
        p.insertAdjacentHTML("afterbegin", "Veuillez entrer un mot de passe valide");
    ...
    .then(result => { 
        ...
        if (result.error || result.message) {
            const p = document.createElement("p");
            p.insertAdjacentHTML("afterbegin", "La combinaison e-mail/mot de passe est incorrecte");
            loginMdpError.appendChild(p);
